...
create table posts(
  ...
  img varchar(255) not null,
  created_at date default current_date,
   uid_idx INTEGER REFERENCES users(user_id)
); 
...
ALTER TABLE orders
     ALTER TABLE users ADD CONSTRAINT fk_posts_users FOREIGN KEY ( uid_idx) REFERENCES users (user_id) on delete cascade
   on update cascade;
