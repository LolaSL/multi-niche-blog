    ...
    <Navbar className="navbar nav" expand="lg">
      <Container className="container">
        ...
        >
          ...
          {isAuth ? (
            <Link className="link" to={`/login?redirect=${redirect}`} onClick={() => logout()}>Logout</Link>
          ) : (
            <div>
              <Link to={`/login?redirect=${redirect}`} className="link" > Login</Link>
            </div>
          )}
          <span className="create mt-2">
            <div>
              <Link to={`/createpost?redirect=${redirect}`} className="link" >Create</Link>
            </div>
          </span>
        </Navbar.Collapse>
      </Container>
